...
#[async_trait]
impl FundingDataProvider for BinanceClient {
    fn venue(&self) -> Venue {
        Venue::Binance
    }

    fn funding_period_hours(&self) -> u32 {
        8 // Binance settles funding every 8 hours
    }
    ...
    async fn get_venue_assets(&self) -> anyhow::Result<Vec<VenueAsset>> {
        ...
        // Build lookup maps
        let volume_map: std::collections::HashMap<String, rust_decimal::Decimal> = tickers
            .iter()
            .map(|t| (t.symbol.clone(), t.quote_volume))
            .collect();

        let price_map: std::collections::HashMap<String, rust_decimal::Decimal> = tickers
            .iter()
            .map(|t| (t.symbol.clone(), t.last_price))
            .collect();

        let spread_map: std::collections::HashMap<String, rust_decimal::Decimal> = book_tickers
            .iter()
            .filter_map(|b| {
                if b.ask_price > rust_decimal::Decimal::ZERO {
                    let spread = (b.ask_price - b.bid_price) / b.ask_price;
                    Some((b.symbol.clone(), spread))
                } else {
                    None
                }
            })
        ...
        Ok(funding_rates
            ...
            .filter_map(|fr| {
                ...
                let base_asset = fr.symbol.strip_suffix("USDT")?.to_string();
                let volume = *volume_map.get(&fr.symbol)?;
                let price = *price_map.get(&fr.symbol)?;
                let spread = spread_map.get(&fr.symbol).copied();

                let mut asset = VenueAsset::new(
                    fr.symbol,
                    base_asset,
                    fr.funding_rate, // Already 8h for Binance
                    volume,
                    price,
                );

                if let Some(s) = spread {
                    asset = asset.with_spread(s);
                }
